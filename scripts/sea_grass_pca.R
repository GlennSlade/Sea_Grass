...
rds <-read.csv("E:/Glenn/Philleigh/R_Scripts/Sea_Grass/data_out/reflectance_extract_Philleigh_pca2.csv")


rds.pca <- prcomp(rds[,c(2,3,4,5,6,7)], center = TRUE,scale. = TRUE)
#rds.pca <- prcomp(rds, center = TRUE,scale. = TRUE)
...
ggbiplot(rds.pca)
ggbiplot(rds.pca,ellipse=TRUE,  labels=rds$Type, groups=rds$Type)
ggbiplot(rds.pca,ellipse=TRUE, choices=c(3,4), labels=rds$Type, groups=rds$Type)


write.csv(rds.pca$rotation, "E:/Glenn/Philleigh/R_Scripts/Sea_Grass/data_out/pca_pcrcomp_rot.csv", row.names=TRUE)
write.csv(rds.pca$sdev, "E:/Glenn/Philleigh/R_Scripts/Sea_Grass/data_out/pca_pcrcomp_std.csv", row.names=TRUE)

# plot histograms of PCA
pca2 <- read.csv("E:/Glenn/Philleigh/R_Scripts/Sea_Grass/data_out/pca2std.csv")
head (pca2)
